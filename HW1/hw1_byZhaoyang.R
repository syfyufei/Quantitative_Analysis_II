...
setwd("C:/Users/lenovo/Desktop/政治数据分析/第一周")
...
axis(2, at = seq(0,150,by=50),las=2)
...
axis(1, at = seq(60,140,by=20))
axis(2, at = seq(0,150,by=50))
curve(coef(M3)[1] + coef(M3)[2] + coef(M3)[3]*x, from = min(mom_iq[mom_hs==1]), to = max(mom_iq[mom_hs==1]), add = TRUE)
...
text(x=min(mom_iq[mom_hs==1]), y=coef(M4)[1] + coef(M4)[2] + coef(M4)[3]*min(mom_iq[mom_hs==1]) + coef(M4)[4]*min(mom_iq[mom_hs==1]), "mom_hs==1", adj=1)
text(x=min(mom_iq[mom_hs==0]), y=coef(M4)[1] + coef(M4)[3]*min(mom_iq[mom_hs==0]), "mom_hs==0", adj=1, col=2)

# p25
stdize <- function(x){
    u <- x - mean(x)#(x, na.rm=TRUE)考虑缺失值
    se <- sd(x)
    newX <- u/se
    return(newX)
}
